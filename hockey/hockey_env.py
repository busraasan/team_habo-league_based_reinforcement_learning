...
class Mode(Enum):
    ...
    TRAIN_SHOOTING = 1
    TRAIN_DEFENSE = 2
    TRAIN_ATTACK = 3  # <--- New Mode for training attacks
...
class HockeyEnv(gym.Env, EzPickle):
  ...
  def reset(self, one_starting=None, mode=None, seed=None, options=None):
    ...
    self.player1_shoot = False # modified
    self.player2_shoot = False # modified
    self.puck_has_been_in_player1_side = False # modified
    self.puch_has_been_in_player2_side = False # modified
    self.time = 0
    ...
    if self.mode == Mode.NORMAL:
        ...
        if one_starting is not None:
            ...
            self.one_starts = not self.one_starts
    elif self.mode == Mode.TRAIN_SHOOTING:
        self.max_timesteps = 80
        # existing logic ...
    elif self.mode == Mode.TRAIN_DEFENSE:
        self.max_timesteps = 150
        # existing logic ...
    elif self.mode == Mode.TRAIN_ATTACK:
        self.max_timesteps = 150  # or any suitable limit
        # -------------- Attack-specific logic --------------
        self.one_starts = True  # Puck always starts on Player 1 side
    # -----------------------------------------------------
    self.closest_to_goal_dist = 1000
    ...
    if self.mode == Mode.NORMAL:
      ...
      w_random = self.r_uniform(W / 7, W * 3/ 7)
      h_random = self.r_uniform(H / 5, 4 * H / 5)
      self.player1 = self._create_player(
        (w_random, h_random),
        red,
        False
    ...
    if self.mode == Mode.NORMAL:
        ...
        w_random = self.r_uniform(4 * W / 7, 6 * W / 7)
        h_random = self.r_uniform(H / 5, 4 * H / 5)
        self.player2 = self._create_player(
            (w_random, h_random),
            blue,
    ...
    # if self.mode == Mode.TRAIN_ATTACK:
    #     self.player2 = self._create_player((5 * W / 6, H / 2), blue, True)
    ...
    if self.mode == Mode.TRAIN_ATTACK:
        puck_x = self.r_uniform(W/4, W/3)
        puck_y = self.r_uniform(H/2 - 0.2, H/2 + 0.2)
        self.puck = self._create_puck((puck_x, puck_y), (0, 0, 0))

        # Optionally give the puck some velocity to simulate pass / rebound
        # 1/2 prob it stay still and 1/2 prob it moves
        if self.r_uniform(0, 1) < 0.5:
            vx = self.r_uniform(-1.0, 1.0)  # slow forward velocity
            vy = self.r_uniform(-1.0, 1.0)
            self.puck.linearVelocity = (vx, vy)
        else:
            self.puck.linearVelocity = (0, 0)
    elif self.mode == Mode.NORMAL or self.mode == Mode.TRAIN_SHOOTING:
    ...
    #   self.puck.ApplyForceToCenter(force, True)
      puck_x = self.r_uniform(W* 2/3, W * 3/4)
      puck_y = self.r_uniform(H/2 - 0.2, H/2 + 0.2)
      self.puck = self._create_puck((puck_x, puck_y), (0, 0, 0))
  ...
  def _compute_reward(self):
    r = 0
    if self.done:
        if self.winner == 0 and self.mode == Mode.TRAIN_ATTACK:
            # Puck returned to half => fail
            r -= 3  # small penalty for failing to score
        elif self.winner == 1:
            r += 10
        elif self.winner == -1:
            r -= 10
        if self.winner == 0 and self.mode == Mode.TRAIN_DEFENSE and self.player2_shoot and self.puck_has_been_in_player1_side:
            r += 3  # small reward for successful defense            
  ...
  def get_reward(self, info): # old reward
    r = self._compute_reward()
    # r += info["reward_closeness_to_puck"]
    
    # 4) Defense shaping
    # If puck is in our half and heading toward us
    if self.mode == Mode.TRAIN_DEFENSE:
      puck_vel_x = self.puck.linearVelocity[0]
      if self.puck.position[0] < CENTER_X and puck_vel_x <= 0:
          dist_player_to_puck = dist_positions(self.player1.position, self.puck.position)
          r -= 0.02 * dist_player_to_puck  # small penalty if far from puck on defense
          
    # reward for shooting           
    if self.mode == Mode.TRAIN_SHOOTING:
      if self.keep_mode and self.player1_shoot and self.player1_has_puck > 3:
        r += 0.05
      # 2) Puck possession shaping
      # If agent is currently "keeping" the puck
      if self.player1_has_puck > 1:
          r += 0.05  # small continuous reward per step of possession
      
    return float(r)

  # def get_reward(self, info):
  #     # 1) Existing final reward
  #     r = self._compute_reward()  # +10 or -10 or 0
  ...
  #     # 3) Distance-based shaping (puck progress)
  #     # Suppose we store self.old_dist_to_goal each step
  #     dist_new = dist_positions(self.puck.position, (W, H/2))  # distance to right goal
  #     if hasattr(self, 'old_dist_to_goal'):
  #         r += 0.1 * (self.old_dist_to_goal - dist_new)
  #     self.old_dist_to_goal = dist_new
  ...
  #     # 5) (Optional) Reward for shooting after 3+ steps of possession
  #     # if agent shot in this step:
  #     #   r += 0.2
  #     # 4) Shooting
  #     if self.keep_mode and self.player1_shoot and self.player1_has_puck > 3:
  #         r += 0.1
  ...
  def get_reward_agent_two(self, info_two):
    ...
    if self.mode == Mode.TRAIN_SHOOTING:
      puck_vel_x = self.puck.linearVelocity[0]
      if self.puck.position[0] > CENTER_X and puck_vel_x >= 0:
          dist_player_to_puck = dist_positions(self.player2.position, self.puck.position)
          r -= 0.02 * dist_player_to_puck  # small penalty if far from puck on defense

    if self.mode == Mode.TRAIN_DEFENSE:
      # give a reward if player2 shot
      if self.keep_mode and self.player2_shoot and self.player2_has_puck > 3:
        r += 0.02
      # 2) Puck possession shaping
      # If agent is currently "keeping" the puck
      if self.player2_has_puck > 1:
          r += 0.01  # small continuous reward per step of possession
    return float(r)
  ...
  def step(self, action):
    ...
    if self.puck.position[0] < CENTER_X:
        self.puck_has_been_in_player1_side = True
        
    if self.puck.position[0] > CENTER_X:
        self.puch_has_been_in_player2_side = True
        
    
    if self.mode == Mode.TRAIN_ATTACK:
        # 1) End if puck is back in Player 1 half-court
        #    i.e. puck's x < W / 2 by some margin (like < CENTER_X - 0.1 if you want a buffer)
        if self.puck.position[0] < CENTER_X - 0.1 and self.puch_has_been_in_player2_side:
            # Attack failed
            self.done = True
            self.winner = 0  # or some marker for "no one scored"
    
    if self.mode == Mode.TRAIN_DEFENSE:
        # 1) End if puck is back in Player 2 half-court
        if (self.puck.position[0] > CENTER_X + 0.1 and self.puck_has_been_in_player1_side) or self.player1_shoot :
            # Defense failed
            self.done = True
            self.winner = 0
